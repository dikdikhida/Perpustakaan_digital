...
if menu == "Kelola Buku":
    ...
    if st.button("Urutkan Berdasarkan Judul"):
        ...
        if hasil:
            st.dataframe(pd.DataFrame([b.to_dict() for b in hasil]), hide_index=True)
        else:
            st.info("Data kosong")
    ...
    with col1:
        ...
        if st.button("Tampilkan Traversal"):
            ...
            data = func[opsi]()
            if data:
                st.dataframe(pd.DataFrame([b.to_dict() for b in data]), hide_index=True)
            else:
                st.info("Tree kosong")
    with col2:
        st.divider()
