...
class Buku:
    def __init__(self, id_buku, judul, penulis, tahun): self.id = id_buku; self.judul = judul; self.penulis = penulis; self.tahun = tahun
    def to_dict(self): return {"ID": self.id, "Judul": self.judul, "Penulis": self.penulis, "Tahun": self.tahun}
class Node:
    def __init__(self, data): self.data = data; self.next = None
class SingleLinkedList:
    def __init__(self): self.head = None
    def kosong(self): return self.head is None
    def tambah_buku(self, buku):
        node = Node(buku)
        if self.kosong(): self.head = node
        else:
            bantu = self.head
    ...
    def to_list(self):
        ...
        while bantu: data.append(bantu.data); bantu = bantu.next
        return data
    def cari_id(self, id_buku):
        bantu = self.head
        while bantu:
            if bantu.data.id == id_buku: return bantu.data
            bantu = bantu.next
        return None
    def hapus(self, id_buku):
        ...
        while sekarang:
            if sekarang.data.id == id_buku:
                if sebelum is None: self.head = sekarang.next
                else: sebelum.next = sekarang.next
                return True
            sebelum, sekarang = sekarang, sekarang.next
        return False
    def edit(self, id_buku, judul, penulis, tahun):
        buku = self.cari_id(id_buku)
        if buku: buku.judul, buku.penulis, buku.tahun = judul, penulis, tahun; return True
        return False
...
class DoubleLinkedList:
    ...
    def tambah(self, data):
        baru = DNode(data)
        if self.head is None: self.head = baru; return
        bantu = self.head
        while bantu.next: bantu = bantu.next
        bantu.next = baru; baru.prev = bantu
    def to_list(self):
        data, bantu, no = [], self.head, 1
        while bantu: data.append(f"{no}. {bantu.data}"); bantu = bantu.next; no += 1
        return data

class QueueNode:
    def __init__(self, data): self.data = data; self.next = None
class Queue:
    def __init__(self): self.front = None; self.rear = None
    def kosong(self): return self.front is None
    def enqueue(self, data):
        baru = QueueNode(data)
        if self.kosong(): self.front = baru; self.rear = baru
        else: self.rear.next = baru; self.rear = baru
    def dequeue(self):
...
class CircularSingle:
    ...
    def tambah(self, data):
        baru = CSNode(data)
        if self.head is None: self.head = baru; baru.next = baru
        else:
            bantu = self.head
            while bantu.next!= self.head: bantu = bantu.next
            bantu.next = baru; baru.next = self.head
    def to_list(self):
...
class CircularDouble:
    ...
    def tambah(self, data):
        baru = CDNode(data)
        if self.head is None: self.head = baru; baru.next = baru; baru.prev = baru
        else:
            tail = self.head.prev
            tail.next = baru; baru.prev = tail
            baru.next = self.head; self.head.prev = baru
    def to_list(self):
        if not self.head: return []
        data, bantu, no = [], self.head, 1
        while True: 
...
class TreeNode:
    def __init__(self, buku): self.buku = buku; self.left = None; self.right = None
class BinarySearchTree:
    def __init__(self): self.root = None
    def insert(self, buku):
        def _insert(node, buku):
            if buku.id < node.buku.id:
                if node.left is None: node.left = TreeNode(buku)
                else: _insert(node.left, buku)
            elif buku.id > node.buku.id:
                if node.right is None: node.right = TreeNode(buku)
                else: _insert(node.right, buku)
        if self.root is None: self.root = TreeNode(buku)
        else: _insert(self.root, buku)
    def search(self, id_buku):
        def _search(node, id_buku):
            if node is None: return None
            if node.buku.id == id_buku: return node.buku
    ...
    def inorder_list(self):
        res = []
        def _in(node):
            if node: _in(node.left); res.append(node.buku); _in(node.right)
        _in(self.root); return res
    def preorder_list(self):
        res = []
        def _pre(node):
            if node: res.append(node.buku); _pre(node.left); _pre(node.right)
        _pre(self.root); return res
    def postorder_list(self):
        res = []
        def _post(node):
            if node: _post(node.left); _post(node.right); res.append(node.buku)
    ...
    for _, row in df.iterrows():
        try:
            ...
            if not inventaris.cari_id(id_buku):
                ...
                inventaris.tambah_buku(buku); tree.insert(buku); count_baru += 1
        except: continue
    return count_baru
...
def linear_search(data, keyword): return [b for b in data if keyword.lower() in b.judul.lower()]
def binary_search(data, keyword):
    kiri, kanan = 0, len(data)-1
    while kiri <= kanan:
        tengah = (kiri+kanan)//2
        if data[tengah].judul.lower() == keyword.lower(): return [data[tengah]] # <- FIX 1: BUNGKUS JADI LIST
        elif keyword.lower() < data[tengah].judul.lower(): kanan = tengah-1
        else: kiri = tengah+1
    return [] # <- FIX 2: JANGAN NONE, TAPI []
...
def quick_sort(data): return sorted(data, key=lambda x: x.judul)

with st.sidebar:
    st.title(f"Halo, {st.session_state.get('username', 'Admin')}") # <- FIX 3: PAKE.GET()
    menu = st.selectbox("Menu", ["Kelola Buku", "Peminjaman", "Pengembalian", "Searching", "Sorting", "Tree", "Laporan"])
    if st.button("Logout"): 
        st.session_state.logged_in = False 
        st.session_state.username = "" # <- KOSONGIN
        st.rerun()
...
if menu == "Kelola Buku":
    ...
    with tab1:
        with st.form(f"form_tambah_{st.session_state.form_key}", clear_on_submit=True):
            ...
            with col1: id_buku_str = st.text_input("ID Buku", value="", placeholder="Masukkan ID Angka")
            with col2: tahun = st.number_input("Tahun Terbit", step=1, min_value=1900, max_value=2026, value=None, placeholder="Contoh: 2024")
            ...
            if st.form_submit_button("Tambah Buku"):
                ...
                if not judul or not penulis or id_buku is None or tahun is None: st.error("Semua field wajib diisi")
                elif inventaris.cari_id(id_buku): st.error("ID sudah ada")
                else: 
    ...
    with tab2:
        ...
        if data:
            ...
            col1, col2 = st.columns(2)
            with col1: st.download_button("Download CSV", data=convert_df_to_csv(df), file_name="data_buku.csv", mime="text/csv")
            with col2:
                ...
                if uploaded_file:
                    try:
                        ...
                        if st.button("Import Data Ini"):
                            count = import_buku_from_df(df_upload); st.success(f"Berhasil import {count} buku baru"); st.rerun()
                    except Exception as e: st.error(f"Gagal baca file: {e}")
        else: st.info("Belum ada buku. Silakan tambah buku di tab 'Tambah'.")
    ...
    with tab3:
        ...
        if id_edit_str and id_edit is None: st.error("ID harus angka >= 1")
        elif buku:
    ...
    with tab4:
        ...
        if id_hapus_str and id_hapus is None: st.error("ID harus angka >= 1")
        if st.button("Hapus Buku"):
            if id_hapus is None: st.error("Masukkan ID yang valid dulu")
            elif inventaris.hapus(id_hapus): st.success("Buku berhasil dihapus"); st.rerun()
    ...
    with col1:
        ...
        if st.button("Pinjam"):
            if nama and judul: antrian.enqueue(f"{nama} meminjam '{judul}'"); favorit.tambah(judul); st.success("Masuk antrian"); st.rerun()
            else: st.warning("Nama dan Judul wajib diisi")
    with col2: st.write("#### Antrian Peminjaman"); st.code("\n".join(antrian.to_list()) or "Antrian kosong.")
    ...
    with col1:
        if st.button("Proses Pengembalian Teratas"):
            ...
            if data: riwayat.tambah(data); st.success(f"Diproses: {data}"); st.rerun()
            else: st.warning("Tidak ada antrian.")
    with col2: st.write("#### Riwayat Pengembalian"); st.code("\n".join(riwayat.to_list()) or "Belum ada riwayat.")
    ...
    if st.button("Cari"):
        ...
        if not key: st.warning("Masukkan keyword dulu")
            ...
            if hasil: st.dataframe(pd.DataFrame([b.to_dict() for b in hasil]), hide_index=True, use_container_width=True)
            else: st.warning("Tidak ditemukan.")
        else:
            data_sorted = merge_sort(data)
            hasil = binary_search(data_sorted, key)
            if hasil: st.dataframe(pd.DataFrame([b.to_dict() for b in hasil]), hide_index=True, use_container_width=True) # <- UDAH AMAN
            else: st.warning("Tidak ditemukan.")
    ...
    with col2:
        ...
        if id_cari_str and id_cari is None: st.error("ID harus angka >= 1")
    ...
    with col2:
        st.write("#### Antrian Peminjaman"); st.code("\n".join(antrian.to_list()) or "-")
        st.write("#### Riwayat Pengembalian"); st.code("\n".join(riwayat.to_list()) or "-")
        st.write("#### Rekomendasi Buku"); st.code("\n".join(rekomendasi.to_list()) or "-")
        st.divider()
        if st.button("Reset Semua Data", type="primary"): st.session_state.clear(); st.rerun()
